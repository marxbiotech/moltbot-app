...
| `AGENTID_UNRESOLVED` | `agentId` was passed to `run_coder` but does not match any roster entry. The tool now rejects the call rather than silently falling back to defaults. | 「找不到 agentId="{id}"。請呼叫 `coding_agents_list` 查看可用代號，或直接提供 cwd 與 agent 參數。」 |
