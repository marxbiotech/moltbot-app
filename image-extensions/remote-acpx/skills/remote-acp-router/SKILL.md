...
If the user explicitly designated a registered executor alias — `cc`, `cx`, or any agent name from the Aliases section — in executor position (e.g. "讓 cc 去執行 …", "用 cx 跑 …", "cc 幫我做 …"), **skip the resolution chain entirely**:
