...
debug.get('/env', async (c) => {
  return c.json({
    ...
    anthropic_key_suffix: c.env.ANTHROPIC_API_KEY?.slice(-6) || null,
    has_openai_key: !!c.env.OPENAI_API_KEY,
    openai_key_suffix: c.env.OPENAI_API_KEY?.slice(-6) || null,
    has_google_key: !!c.env.GOOGLE_API_KEY,
    google_key_suffix: c.env.GOOGLE_API_KEY?.slice(-6) || null,
